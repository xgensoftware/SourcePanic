...
import com.slobodastudio.smspanic.activities.WidgetConfigureActivity;
...
public class SmsWidgetProvider extends AppWidgetProvider {
	...
	public void onDisabled(final Context context, final int[] appWidgetIds) {
		...
		pm.setComponentEnabledSetting(new ComponentName("com.slobodastudio.smspanic",
				".widget.SmsWidgetProvider"), PackageManager.COMPONENT_ENABLED_STATE_DISABLED,
	...
	public void onEnabled(final Context context) {
		...
		PackageManager pm = context.getPackageManager();
		pm.setComponentEnabledSetting(new ComponentName("com.slobodastudio.smspanic",
				".widget.SmsWidgetProvider"), PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
				PackageManager.DONT_KILL_APP);
	}
	...
	private void updateAppWidget(final Context context, final AppWidgetManager appWidgetManager,
		...
		PanicWidgetModel pwm = PanicWidgetModel.retrieveModel(context, appWidgetId);
		if (pwm == null) {
			return;
		}
		WidgetConfigureActivity.updateAppWidget(context, appWidgetManager, pwm, pwm.getNum(), pwm.getText(),
				pwm.getMessageId(), pwm.getWidgetName(), pwm.getClickOnes());
	}
}
